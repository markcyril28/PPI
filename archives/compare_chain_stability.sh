...
# Output directory
WORKDIR="/mnt/local3.5tb/home/mcmercado/PPI/results_gromacs"
...
# Number of CPU threads (optimal for GPU: 16-32, CPU-only: 32-64)
# With 128 threads available, using 16 threads with GPU is optimal for this system size
NTHREADS=128


# GPU acceleration (set to 'auto' for automatic GPU detection, or '0,1,2...' for specific GPUs)
GPU_ID="auto"
...
check_gromacs() {
    if ! command -v gmx &> /dev/null; then
        log_error "GROMACS (gmx) not found in PATH"
        log_error "Please source GROMACS: source /path/to/gromacs/bin/GMXRC"
        exit 1
    fi
    log_info "GROMACS version: $(gmx --version 2>&1 | head -1)"
    
    # Detect GPU availability
    if gmx -version 2>&1 | grep -q "GPU support.*enabled"; then
        log_info "GPU support: ENABLED"
        log_info "Using $NTHREADS CPU threads with GPU acceleration"
    else
        log_info "GPU support: DISABLED (CPU-only mode)"
        if [ "$NTHREADS" -lt 32 ]; then
            log_info "Increasing threads to 32 for CPU-only execution"
            NTHREADS=32
        fi
    fi
}
...
process_structure() {
    ...
    echo "1" | gmx pdb2gmx -f clean.pdb -o protein.gro -water $WATERMODEL -ff $FORCEFIELD -ignh 2>&1 | tee pdb2gmx.log
    ...
    gmx editconf -f protein.gro -o boxed.gro -c -d $BOX_DISTANCE -bt dodecahedron 2>&1 | tee editconf.log
    
    # Solvate
    log_info "Solvating the system..."
    gmx solvate -cp boxed.gro -cs spc216.gro -o solvated.gro -p topol.top 2>&1 | tee solvate.log
    
    # Add ions
    log_info "Adding ions..."
    create_em_mdp "ions.mdp"
    gmx grompp -f ions.mdp -c solvated.gro -p topol.top -o ions.tpr -maxwarn 2 2>&1 | tee grompp_ions.log
    echo "SOL" | gmx genion -s ions.tpr -o ionized.gro -p topol.top -pname NA -nname CL -neutral -conc $ION_CONCENTRATION 2>&1 | tee genion.log
    
    # Energy minimization
    log_info "Running energy minimization..."
    create_em_mdp "em.mdp"
    gmx grompp -f em.mdp -c ionized.gro -p topol.top -o em.tpr 2>&1 | tee grompp_em.log
    
    local gmx_cmd="gmx mdrun -v -deffnm em -nt $NTHREADS"
    if [ "$GPU_ID" != "" ]; then
        if [ "$GPU_ID" = "auto" ]; then
            # Let GROMACS auto-detect and use available GPUs
            gmx_cmd="$gmx_cmd -nb gpu -pme gpu -bonded cpu"
        else
            gmx_cmd="$gmx_cmd -gpu_id $GPU_ID -nb gpu -pme gpu -bonded cpu"
        fi
    fi
    $gmx_cmd 2>&1 | tee mdrun_em.log
    
    # Extract EM potential energy
    echo "Potential" | gmx energy -f em.edr -o em_potential.xvg 2>&1 | tee energy_em.log
    
    # NVT equilibration
    log_info "Running NVT equilibration..."
    create_nvt_mdp "nvt.mdp"
    gmx grompp -f nvt.mdp -c em.gro -r em.gro -p topol.top -n index.ndx -o nvt.tpr 2>&1 | tee grompp_nvt.log
    
    gmx_cmd="gmx mdrun -v -deffnm nvt -nt $NTHREADS"
    if [ "$GPU_ID" != "" ]; then
        if [ "$GPU_ID" = "auto" ]; then
            gmx_cmd="$gmx_cmd -nb gpu -pme gpu -bonded cpu"
        else
    ...
    gmx grompp -f npt.mdp -c nvt.gro -r nvt.gro -t nvt.cpt -p topol.top -n index.ndx -o npt.tpr 2>&1 | tee grompp_npt.log
    
    gmx_cmd="gmx mdrun -v -deffnm npt -nt $NTHREADS"
    if [ "$GPU_ID" != "" ]; then
        if [ "$GPU_ID" = "auto" ]; then
            gmx_cmd="$gmx_cmd -nb gpu -pme gpu -bonded cpu"
        else
    ...
    gmx grompp -f md.mdp -c npt.gro -t npt.cpt -p topol.top -n index.ndx -o md.tpr 2>&1 | tee grompp_md.log
    
    gmx_cmd="gmx mdrun -v -deffnm md -nt $NTHREADS"
    if [ "$GPU_ID" != "" ]; then
        if [ "$GPU_ID" = "auto" ]; then
            gmx_cmd="$gmx_cmd -nb gpu -pme gpu -bonded cpu"
        else
...
create_chain_index() {
    ...
    echo "q" | gmx make_ndx -f protein.gro -o index.ndx 2>&1 | tee make_ndx.log
...
calculate_interaction_energy() {
    ...
    gmx grompp -f ie.mdp -c md.gro -p topol.top -n index.ndx -o ie.tpr -maxwarn 5 2>&1 | tee grompp_ie.log
    
    # Rerun trajectory to calculate energies
    log_info "Rerunning trajectory for energy calculation..."
    gmx mdrun -rerun md.xtc -s ie.tpr -e ie.edr -g ie.log 2>&1 | tee mdrun_ie.log
    ...
    echo -e "Coul-SR:ChainA-ChainB\nLJ-SR:ChainA-ChainB\n\n" | \
        gmx energy -f ie.edr -o interaction_energy.xvg 2>&1 | tee energy_ie.log || true
    ...
    if [ ! -s "interaction_energy.xvg" ]; then
        ...
        echo -e "Backbone\nBackbone" | gmx rms -s md.tpr -f md.xtc -o rmsd.xvg 2>&1 | tee rmsd.log
        
        # Calculate radius of gyration
        echo "Protein" | gmx gyrate -s md.tpr -f md.xtc -o gyrate.xvg 2>&1 | tee gyrate.log
        
        # Calculate RMSF
        echo "Backbone" | gmx rmsf -s md.tpr -f md.xtc -o rmsf.xvg -res 2>&1 | tee rmsf.log
        
        # Calculate hydrogen bonds between chains
        echo -e "ChainA\nChainB" | gmx hbond -s md.tpr -f md.xtc -n index.ndx -num hbonds.xvg 2>&1 | tee hbond.log || true
        
        # Calculate contact map / minimum distance between chains
        echo -e "ChainA\nChainB" | gmx mindist -s md.tpr -f md.xtc -n index.ndx -od mindist.xvg 2>&1 | tee mindist.log || true
    fi
...
calculate_binding_metrics() {
    ...
    echo "System" | gmx trjconv -s md.tpr -f md.xtc -o final_frame.gro -dump 0 -pbc mol 2>&1 || true
    
    # Calculate SASA (Solvent Accessible Surface Area)
    echo "Protein" | gmx sasa -s md.tpr -f md.xtc -o sasa.xvg -or resarea.xvg 2>&1 | tee sasa.log || true
